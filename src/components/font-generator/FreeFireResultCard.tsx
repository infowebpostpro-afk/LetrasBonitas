...
interface FreeFireResultCardProps {
  id: string;
  name: string;
  categoryLabel: string;
  isFavorite: boolean;
  onToggleFavorite: (id: string, name: string) => void;
  onSelectForCustomization?: (name: string) => void;
}

export const FreeFireResultCard: React.FC<FreeFireResultCardProps> = ({
  id,
  name,
  categoryLabel,
  isFavorite,
  ...
  return (
    <div className="bg-white rounded-xl p-4 border border-slate-200/80 hover:border-teal-500/50 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between group">
      <div className="flex items-center justify-between mb-3">
        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-50 text-teal-800 border border-teal-200/60">
          {categoryLabel}
        </span>
        ...
        <div className="flex items-center gap-1">
          {onSelectForCustomization && (
            <button
              onClick={() => onSelectForCustomization(name)}
              title="Personalizar este apodo"
              className="p-1.5 rounded-lg text-slate-400 hover:text-teal-600 hover:bg-teal-50 transition-colors"
              aria-label="Personalizar apodo"
            >
      ...
      <div className="my-2 min-h-[3.25rem] flex items-center justify-center p-2 rounded-lg bg-slate-50 border border-slate-100 group-hover:bg-teal-50/30 transition-colors overflow-x-auto max-w-full">
